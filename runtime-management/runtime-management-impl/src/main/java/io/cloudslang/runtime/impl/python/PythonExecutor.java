...
public class PythonExecutor implements Executor {
  public static String THREADED_MODULES_ISSUE="No module named";
  private static Logger logger=Logger.getLogger(PythonExecutor.class);
  private static String TRUE="true";
  private static String FALSE="false";
  private static PythonInterpreter GLOBAL_INTERPRETER=new ThreadSafePythonInterpreter(null);
  /** 
 * There is an issue in loaded environment - existing python module not found in PySystem.modules.table although it exists in the table. Meanwhile we execute retries ans will open an issue to the jython.org
 */
  public static int RETRIES_NUMBER_ON_THREADED_ISSUE=3;
  public static int MAX_LENGTH=Integer.getInteger("input.error.max.length",1000);
static {
  ...
  public PythonEvaluationResult eval(  String prepareEnvironmentScript,  String expr,  Map<String,Serializable> context){
    ...
    try {
      ...
      throw new RuntimeException("Error in running script expression: '" + exprSubstring + "',\n\tException is: "+ handleExceptionSpecialCases(exception.value.toString()),exception);
    }
  ...
  public Serializable eval(  String prepareEnvironmentScript,  String script){

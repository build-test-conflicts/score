...
/** 
 * User: stoneo Date: 22/07/2014 Time: 14:42
 */
@RunWith(SpringJUnit4ClassRunner.class) @ContextConfiguration(locations="classpath:/META-INF/spring/samples/schemaAllTestContext.xml") @DirtiesContext(classMode=DirtiesContext.ClassMode.AFTER_CLASS) public class StandAloneTest {
  ...
  private static Logger logger=Logger.getLogger(StandAloneTest.class);
  private static String simpleNavigationMethodName="simpleNavigation";
  private static String navigationActionClassName="io.cloudslang.samples.controlactions.NavigationActions";
  ...
  private ScoreEvent getEventFromQueueByType(  String eventType){
    for (    ScoreEvent event : eventQueue) {
      if (event.getEventType().equals(eventType))       return event;
    }
    return null;
  }
  ...
  private static ExecutionStep createPutDataOnSessionStep(  Long stepId,  Long nextStepId){
    ...
    if (nextStepId != null)     addNavigationToExecutionStep(nextStepId,navigationActionClassName,simpleNavigationMethodName,executionPutDataStep);
